...
function init() {
    ...
    gui = new dat.GUI();
 gui.add(Configs, 'globalAlpha', .2, 1.0);
  
  gui.add(Configs,'particleNum', 20, 3000);
  
    gui.add(Configs, 'step', 1, 10);
    gui.add(Configs, 'base', 500, 3000);
    gui.add(Configs, 'zInc', 0.0001, 0.01);
   gui.add(Configs, 'octaves',2, 40);
  gui.add(Configs, 'fallout', .2, 2.0);
  gui.add(Configs, 'amp', 1, 20.0);
    gui.open();
...
function onCanvasClick(e) {
    context.save();
    context.globalAlpha = Configs.globalAlpha;
...
function getNoise(x, y, z) {
    var octaves = Configs.octaves,
        fallout = Configs.fallout,
        amp = Configs.amp, f = 1, sum = 0,
        i;
